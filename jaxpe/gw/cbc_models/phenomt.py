...
class IMRPhenomT(TimeDomainModel):
    ...
    def _pn_omega(self, eta, chi1, chi2):
        ...
        w = w.at[3].set(
            -3.0 * PI / 10.0
            + 113.0 / 160.0 * (m1 * chi1 + m2 * chi2)
            - 19.0 / 80.0 * eta * (chi1 + chi2)
        )

        w4 = (
            1855099.0 / 14450688.0
            - 243.0 * (m1 * chi1**2 + m2 * chi2**2) / 1024.0
            + 56975.0 * eta / 258048.0
            + 3.0
            * eta
            * (81.0 * chi1**2 - 158.0 * chi1 * chi2 + 81.0 * chi2**2)
            / 1024.0
            + 371.0 * eta**2 / 2048.0
        )
        w = w.at[4].set(w4)
        ...
        w5 = (
            -7729.0 * PI / 21504.0
            + 146597.0 / (64512.0) * (m1 * chi1 + m2 * chi2)
            + 13.0 * PI * eta / 256.0
            - 1213.0 * eta * (chi1 + chi2) / 1152.0
            + 7.0 * eta / 128.0 * dm * (chi1 - chi2)
            - 17.0 / 128.0 * eta**2 * (chi1 + chi2)
        )
        w = w.at[5].set(w5)
    ...
    def _pn_amp(self, eta, chi1, chi2):
        ...
        h = h.at[3].set(
            2.0 * PI
            - 2.0 * (chi1 + chi2) / 3.0
            + 2.0 * dm * (chi1 - chi2) / (3.0 * (m1 + m2))
            + 2.0 / 3.0 * eta * (chi1 + chi2)
        )
        h = h.at[4].set(
            -2173.0 / 1512.0
            - 1069.0 * eta / 216.0
    ...
    def _evaluate_frequency(self, t, eta, chi1, chi2, c):
        ...
        w_merg = (
            c["a0"]
            + c["a1"] * jnp.arcsinh(c["alpha_1"] * t_merg)
            + c["a2"] * jnp.arcsinh(c["alpha_1"] * t_merg) ** 2
            + c["a3"] * jnp.arcsinh(c["alpha_1"] * t_merg) ** 3
            + c["a4"] * jnp.arcsinh(c["alpha_1"] * t_merg) ** 4
        )

        t_rd = jnp.maximum(t, 0.0)
        denom = (
            1.0
            + c["c3"] * jnp.exp(-c["c2"] * t_rd)
            + c["c4"] * jnp.exp(-2.0 * c["c2"] * t_rd)
        )
        num = c["c2"] * (
            c["c3"] * jnp.exp(-c["c2"] * t_rd)
            + 2.0 * c["c4"] * jnp.exp(-2.0 * c["c2"] * t_rd)
        )
    ...
    def _evaluate_amplitude(self, t, omega, eta, chi1, chi2, c):
        ...
        amp_merg = (
            c["b0"]
            + c["b1"] * t_merg**2
            + c["b2"] * (1.0 / jnp.cosh(c["alpha_1"] * t_merg)) ** (1.0 / 7.0)
            + c["b3"] * (1.0 / jnp.cosh(c["alpha_1"] * t_merg))
        )

        t_rd = jnp.maximum(t, 0.0)
        amp_rd = jnp.exp(-c["alpha_1"] * t_rd) * (
            c["d1"] * jnp.tanh(c["d2"] * t_rd + c["d3"]) + c["d4"]
        )

        return jnp.where(
            t <= c["t_meco"], amp_insp, jnp.where(t < 0.0, amp_merg, amp_rd)
        )
