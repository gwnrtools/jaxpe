...
- [Design — Fusing GPry into jaxpe]({{ site.baseurl }}/docs/gpry_fusion_design.html):
...
- [Design — Relative binning]({{ site.baseurl }}/docs/relative_binning_design.html):
  heterodyned likelihoods in the frequency and time domains.
- [Status — Relative binning implementation]({{ site.baseurl }}/docs/relative_binning_status.html):
  what of that design is built and validated.

## Benchmarks

- [BNS PE with FD relative binning + HMC]({{ site.baseurl }}/docs/bns_ce_pe_benchmark.html):
  end-to-end Cosmic Explorer benchmark, a five-kernel comparison, and a mass sweep.
- [TD PE with IMRPhenomT + relative binning]({{ site.baseurl }}/docs/td_phenomt_pe_benchmark.html):
  time-domain pipeline, and the VRAM ceiling it currently hits.

## Experiments

- [Under Construction (Experiments)]({{ site.baseurl }}/docs/under_construction.html)
- [ESIGMA — ISCO & inspiral termination]({{ site.baseurl }}/docs/under_construction_esigma.html)
- [FD IMRPhenomD injection recovery]({{ site.baseurl }}/docs/under_construction_fd.html)
