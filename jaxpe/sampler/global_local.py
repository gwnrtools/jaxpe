"""Global-Local Sampler with Normalizing Flows (flowMC style).

Standard local MCMC kernels (like MALA or HMC) are excellent at exploring a single mode
of a posterior distribution, but they struggle to jump across low-probability valleys
to find other modes. This leads to poor mixing in multi-modal problems.

Motivation & Math
-----------------
To solve this, we augment local MCMC with "global" independence proposals generated by a
Normalizing Flow. A Normalizing Flow learns a parameterized bijective mapping
$f_\phi: X \to Z$ from the complex posterior space $X$ to a simple base distribution
(e.g., standard Gaussian) $Z$.

Once trained on the MCMC samples, the flow serves as a cheap proxy for the posterior:
$q_\phi(x) \approx \pi(x)$. We can draw independent global proposals $y \sim q_\phi$
and accept them via Metropolis-Hastings:
$$ \alpha = \min\left(1, \frac{\pi(y) q_\phi(x)}{\pi(x) q_\phi(y)}\right) $$
Because the flow is trained to match the posterior, this acceptance rate can be reasonably
high, allowing the chains to teleport instantly between isolated modes.
