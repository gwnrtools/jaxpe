...
def Mc_eta_to_ms(params):
    # params = [Mchirp, eta]
    mc = params[0]
    eta = params[1]
    M = mc / (eta ** (3.0 / 5.0))
    m1 = M * 0.5 * (1.0 + jnp.sqrt(1.0 - 4.0 * eta))
    m2 = M * 0.5 * (1.0 - jnp.sqrt(1.0 - 4.0 * eta))
    return m1, m2
...
class IMRPhenomD:
    ...
    def __call__(self, params: dict, freqs: jax.Array):
...
def gen_IMRPhenomD(f, params, f_ref: float):
    """
...
def gen_IMRPhenomD_hphc(f, params, f_ref: float):
    """
    Generate PhenomD frequency domain waveform following 1508.07253.
    vars array contains both intrinsic and extrinsic variables
    theta = [Mchirp, eta, chi1, chi2, D, tc, phic]
