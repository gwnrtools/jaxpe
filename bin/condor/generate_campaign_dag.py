...
def write_dag(
    ...
    if maxjobs:
        lines.append(f"CONFIG {path.parent / 'dagman.config'}")
    ...
    path.write_text("\n".join(lines) + "\n")
    if maxjobs:
        (path.parent / "dagman.config").write_text(
            f"DAGMAN_MAX_JOBS_SUBMITTED = {maxjobs}\n"
        )
...
def main():
    ...
    p.add_argument(
        ...
        type=int,
        default=24,
        help="DAGMAN_MAX_JOBS_SUBMITTED throttle. Default 24: condor_status on this "
        "pool showed ~25-26 total nodes, almost all static whole-node slots (one "
        "job = one whole node regardless of request_cpus), so the real concurrency "
        "ceiling is the node count, not a cores/request_cpus ratio; 24 leaves "
        "1-2 nodes free for other users.",
    )
    args = p.parse_args()
    ...
    print(f"Full campaign (after smoke test succeeds): condor_submit_dag {full_path}")
