...
def parse_sim_inspiral_table(xml_path):
    ...
    if len(tokens) % num_cols != 0:
        raise ValueError(f"Number of tokens ({len(tokens)}) is not a multiple of number of columns ({num_cols}).")
...
def main():
    parser = argparse.ArgumentParser(description="Submit PE runs for injections from an XML file.")
    parser.add_argument("--xml-file", type=str, required=True, help="LIGO_LW XML file containing sim_inspiral table")
    parser.add_argument("--prior-json", type=str, required=True, help="JSON file with prior bounds")
    ...
    parser.add_argument("--scheduler", choices=["local", "slurm", "condor"], default="local", help="How to dispatch jobs")
    parser.add_argument("--run-script", type=str, default="bin/run_pe.py", help="Path to run_pe.py script")
    ...
    for i, inj in enumerate(injections):
        ...
        mapped_inj = {}
        if "mchirp" in inj: mapped_inj["chirp_mass"] = inj["mchirp"]
        ...
        if "distance" in inj: mapped_inj["luminosity_distance"] = inj["distance"]
        if "inclination" in inj: mapped_inj["inclination"] = inj["inclination"]
        if "coa_phase" in inj: mapped_inj["phase"] = inj["coa_phase"]
        ...
        if "eccentricity" in inj: mapped_inj["eccentricity"] = inj["eccentricity"]
        else: mapped_inj["eccentricity"] = 0.0 # Default fallback
        
        if "spin1z" in inj: mapped_inj["spin1z"] = inj["spin1z"]
        if "spin2z" in inj: mapped_inj["spin2z"] = inj["spin2z"]
        if "mean_anomaly" in inj: mapped_inj["mean_anomaly"] = inj["mean_anomaly"]
        else: mapped_inj["mean_anomaly"] = 0.0
        ...
        cmd_args = [
            ...
            "--injection-json", str(inj_json_path),
            "--prior-json", str(prior_json_path),
        ...
        if args.scheduler == "local":
            ...
            env = os.environ.copy()
            env["XLA_FLAGS"] = "--xla_force_host_platform_device_count=16" # Enable multicore
            with open(log_dir / f"{job_name}.out", "w") as fout, open(log_dir / f"{job_name}.err", "w") as ferr:
