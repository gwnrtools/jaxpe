...
class PostProcessor:
    ...
    def __init__(self, problem, raw_samples=None, raw_samples_file=None):
        ...
        if self.samples.ndim != 3:
            raise ValueError(f"Expected samples of shape (n_steps, n_chains, n_dim), got {self.samples.shape}")
    ...
    def extract_independent_samples(self, tau=None, burnin_multiplier=3, min_burnin=0):
        ...
        if burnin >= self.n_steps:
            print("CRITICAL WARNING: Burn-in exceeds total chain length. The sampler did not converge.")
        ...
        print(f"Extracted {flat_samples.shape[0]} independent samples (from {self.n_steps * self.n_chains} raw).")
        return flat_samples
