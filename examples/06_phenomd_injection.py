"""Example: Parameter estimation for an aligned-spin BBH signal using IMRPhenomD.

This script injects a simulated aligned-spin BBH waveform (generated using
the IMRPhenomD model) into Gaussian noise, and runs the GlobalLocal
...
def main(
    ...
    np.save(OUT / "phenomd_injection_samples.npy", flat)
    fig = corner_plot(flat, names=names, truths=truths)
    fig.savefig(OUT / "phenomd_injection_corner.png", dpi=120)
    print(f"\nsaved corner plot to {OUT / 'phenomd_injection_corner.png'}")
