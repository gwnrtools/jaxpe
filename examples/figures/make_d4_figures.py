...
def load_pe():
    ...
    for f in glob.glob(os.path.join(OUT, "phenomd_M*_*.npz")):
        m = re.match(
            r"phenomd_M(\d+)_(gradient_cpu|gradient_gpu|surrogate_cpu|surrogate_jax_cpu|surrogate_jax_gpu)\.npz",
            os.path.basename(f),
        )
        if not m:
            continue
        ...
        pe.setdefault(mass, {})[route] = dict(
            wall=g("wall_seconds"),
            evals=g("likelihood_evaluations"),
            dur=g("duration"),
            wave=g("waveform_seconds"),
            gp=g("gp_seconds"),
...
def fig_eob():
    models = json.load(open(os.path.join(OUT, "phenomd_eob_call_timing.json")))[
        "models"
    ]
    styles = {
        "TEOBResumS": ("#0072B2", "o", "-"),
    ...
    ax.text(
        2.75,
        GPRY_HI * 1.02,
        "GPry overhead per eval (1.4-2.6 s)",
        fontsize=9,
        color="0.25",
        va="bottom",
    )
    ...
    ax.set_yscale("log")
    ax.set_xlabel(
        "total mass  $M_{\\rm tot}$  [$M_\\odot$]   (<- longer signal, higher fs)"
    )
    ax.set_ylabel("waveform generation time per call  [ms]")
    ax.set_title(
        "Real EOB per-call cost vs total mass\n"
...
def fig_pe():
    ...
    bwall = np.array([pe[m].get("surrogate_cpu", {}).get("wall", np.nan) for m in B])
    bwall_jax = np.array(
        [
            pe[m]
            .get("surrogate_jax_gpu", pe[m].get("surrogate_jax_cpu", {}))
            .get("wall", np.nan)
            for m in B
        ]
    )
    bwave = np.array([pe[m].get("surrogate_cpu", {}).get("wave", np.nan) for m in B])
    bgp = np.array([pe[m].get("surrogate_cpu", {}).get("gp", np.nan) for m in B])
    bgp_jax = np.array(
        [
            pe[m]
            .get("surrogate_jax_gpu", pe[m].get("surrogate_jax_cpu", {}))
    ...
    bwall_jax_gpu = np.array(
        [pe[m].get("surrogate_jax_gpu", {}).get("wall", np.nan) for m in B]
    )
    bgp_jax_gpu = np.array(
        [pe[m].get("surrogate_jax_gpu", {}).get("gp", np.nan) for m in B]
    )

    bncall = np.array([pe[m].get("surrogate_cpu", {}).get("ncalls", np.nan) for m in B])
    gp_per_eval = bgp / bncall

    fig, axs = plt.subplots(2, 2, figsize=(11.5, 8.4))
    ax = axs[0, 0]
    ax.plot(
        A, acpu, "o-", color="#0072B2", label="Route A - gradient (CPU)", markersize=6
    )
    ax.plot(
        A, agpu, "s-", color="#009E73", label="Route A - gradient (GPU)", markersize=6
    )
    ax.plot(
        B,
        bwall,
        "D-",
        color="#D55E00",
        label="Route B - GPry surrogate (CPU)",
        markersize=6,
    )
    ax.plot(
        B,
        ...
        "v-",
        color="#E69F00",
        label="Route B - GPry surrogate (JAX CPU)",
        markersize=6,
    )
    ax.plot(
        B,
        ...
        "^-",
        color="#F0E442",
        label="Route B - GPry surrogate (JAX GPU)",
        markersize=6,
    )
    ax.set_yscale("log")
    ...
    ax = axs[1, 0]
    ax.plot(
        B, bgp, "D-", color="#D55E00", label="GP fit + acq sampling (CPU)", markersize=6
    )
    ax.plot(
        B,
        bgp_jax,
        "v-",
        color="#E69F00",
        label="GP fit + acq sampling (JAX CPU)",
        markersize=6,
    )
    ax.plot(
        B,
        ...
        "^-",
        color="#F0E442",
        label="GP fit + acq sampling (JAX GPU)",
        markersize=6,
    )
    ax.plot(
        B,
        ...
        "o-",
        color="#c0392b",
        label="waveform generation (JAX PhenomD)",
        markersize=6,
    )
    ax.set_yscale("log")
    ax.set_xlabel("total mass  $M_{\\rm tot}$  [$M_\\odot$]")
    ...
    fig.suptitle(
        "PhenomD PE scaling with total mass - three routes "
        "(matched SNR~15, 4-D intrinsic)",
        fontsize=13,
        y=1.00,
    )
    fig.tight_layout()
...
def _load_run(mass, route):
    ...
    return dict(
        x=d["samples"].astype(float),
        w=w / w.sum(),
        names=[str(n) for n in d["names"]],
        truth=d["truth"].astype(float),
        dur=float(d["duration"]) if "duration" in d.files else np.nan,
        evals=(
            float(d["likelihood_evaluations"])
            if "likelihood_evaluations" in d.files
            else np.nan
        ),
    )
...
def fig_posteriors():
    ...
    routes = [
        ("gradient_cpu", "#1f6fb2", "Route A – gradient (CPU)"),
        ("gradient_gpu", "#0f8f80", "Route A – gradient (GPU)"),
        ("surrogate_cpu", "#c0392b", "Route B – GPry surrogate (CPU)"),
        ("surrogate_jax_cpu", "#e67e22", "Route B – GPry surrogate (JAX CPU)"),
        ("surrogate_jax_gpu", "#f39c12", "Route B – GPry surrogate (JAX GPU)"),
    ]
    labels = [
        r"chirp mass $\mathcal{M}$",
        "mass ratio $q$",
        r"$\chi_{1z}$",
        r"$\chi_{2z}$",
    ]
    ...
    handles.append(
        Line2D([0], [0], color="#222", ls=(0, (4, 2)), lw=1.2, label="injected truth")
    )
    fig.legend(
        handles=handles,
        loc="upper center",
    ...
    fig.suptitle(
        "Recovered intrinsic posteriors — every run vs truth "
...
def fig_corners():
    ...
    for mass in masses:
        present = [(rt, col, lab, _load_run(mass, rt)) for rt, col, lab in routes]
        ...
        for rt, col, _, v in present:
            ...
            fig = corner.corner(
                ...
                truths=truth if fig is None else None,
                truth_color="#222",
                labelpad=0.08,
                fig=fig,
            )
        handles = [
            Line2D([0], [0], color=col, lw=2.4, label=lab) for _, col, lab, _ in present
        ]
        handles.append(
            Line2D(
                [0], [0], color="#222", ls=(0, (4, 2)), lw=1.3, label="injected truth"
            )
        )
